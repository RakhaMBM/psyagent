# AGENTS.md — инструкция для ИИ-агентов по проекту Psyagent

Этот файл — единая точка входа для любого ИИ (Claude, Cursor, Copilot, ChatGPT и т.п.),
который работает с проектом. Прочитай его целиком перед изменениями. Общение и комментарии — **на русском**.

---

## 1. Что это за проект

**Psyagent** — веб-приложение психологической диагностики студентов для колледжей (педагог-психолог).
Учёт студентов, опросники/методики, прохождение тестов, подсчёт по формуле с интерпретацией,
группа риска, PDF-заключения, статистика, экспорт. **Мультиарендный SaaS** (один сервер —
много колледжей, у каждого своя БД).

- **Backend:** Node.js + Express — весь код в одном файле `server.js`.
- **БД:** MySQL 8 через `mysql2/promise`.
- **Auth:** JWT (`jsonwebtoken`) + `bcryptjs`. Секрет — `JWT_SECRET` (из `.env`, см. §9).
- **Frontend:** статические HTML на Bootstrap 5 в `public/` (без сборки, без фреймворков).
- Внешние библиотеки только в браузере по CDN: Bootstrap 5, Chart.js, SheetJS (xlsx), pdfmake.

---

## 2. Структура

| Файл | Назначение |
|---|---|
| `server.js` | Весь REST API + отдача страниц. ~1500 строк. |
| `schema.sql` | Схема БД колледжа (для чистой установки). |
| `public/index.html` + `index.js` | Вход (поле «код колледжа»). |
| `public/admin.html` + `admin.js` | Кабинет психолога-админа (студенты, кураторы, тесты, методики, результаты, дашборд). |
| `public/curator.html` + `curator.js` | Кабинет куратора (read-only по своей группе). |
| `public/student.html` + `student.js` | Кабинет студента (прохождение тестов). |
| `public/platform.html` + `platform.js` | Кабинет супер-админа (`/platform`): колледжи и управление выбранными пользователями. |
| `public/anonymous.html` + `anonymous.js` | Публичное прохождение анонимных опросов по случайной ссылке. |
| `public/anonymous-surveys.js` | Встроенные анонимные анкеты 1.2.3/1.2.4 без балльного ключа. |
| `public/pdf-download.js` | Общая генерация и скачивание PDF через pdfmake. |
| `public/ai-analysis.js` | UI ИИ-анализа результатов (админ+куратор); серверные роуты `/api/ai/*` и промпты — в `server.js`; включается `OPENWEBUI_*` в `.env`; в LLM уходят только обезличенные данные (без ФИО). |
| `public/event-actions.js` | Делегирование UI-событий через `data-action`; inline-JavaScript запрещён CSP. |
| `public/i18n.js` | Двуязычие RU/KZ (см. §7). |
| `public/methodologies.js` | Движок подсчёта + встроенные методики (см. §5). |
| `public/styles.css` | Общие стили (имя именно `styles.css`). |
| `MULTITENANCY.md` | Детали мультиарендности и серверной настройки БД. |
| `CLAUDE.md` | Краткий гайд (дублирует часть этого файла). |

---

## 3. Роли и вход

Вход: `POST /api/auth/login` с телом `{ username, password, college }`.
- `college` пустой → **колледж по умолчанию** (`default` → БД `psych_diagnostic`). Старый вход не ломается.
- `college = "platform"` → вход **супер-админа** (владелец SaaS, таблица `platform_admins` в control-БД).
- иначе → колледж по коду (таблица `tenants`).

Роли и редиректы (см. `redirectByRole` в `index.html`):
- `super_admin` → `/platform` (cross-tenant, не привязан к колледжу).
- `admin` (психолог) → `/admin` (полный доступ в своём колледже).
- `curator` → `/curator` (read-only **только по своей группе**, `users.group_name`).
- `student` → `/student`.

Middleware в `server.js`: `requireAdmin`, `requireSuperAdmin`, `requireStaff` (admin|curator).

---

## 4. Мультиарендность — КРИТИЧНО

**Отдельная БД на каждый колледж.** Реестр колледжей и супер-админы — в отдельной **control-БД** `psych_control`.

- Колледж зашит в JWT (`tenantDb`). `authenticateToken` ставит `req.db = tenantPool(tenantDb)`.
- **Все маршруты данных колледжа ходят ТОЛЬКО через `req.db`.** НИКОГДА не используй глобальный `pool`
  внутри обработчиков запросов — это утечка данных между колледжами. `pool` = только БД по умолчанию.
- `controlPool` — только для `tenants` / `platform_admins`. `adminPool` (без БД) — для `CREATE DATABASE`.
- `logAction(db, userId, ...)` — первым аргументом передаётся пул колледжа (`req.db` или резолвнутый).
- При провижининге нового колледжа: `CREATE DATABASE` → `ensureTenantSchema(db)` (вся схема `IF NOT EXISTS`)
  → первый админ-психолог. При старте `syncTenantSchemas()` накатывает схему на все активные колледжи.
- **При изменении схемы** правь `ensureTenantSchema()` (а не только `schema.sql`), иначе на боевых
  БД колледжей таблицы/колонки не появятся. Для enum-миграций — идемпотентный `ALTER` в конце `ensureTenantSchema`.

Подробности и шаги настройки БД на сервере (права `CREATE DATABASE` для `psyagent_user` и т.д.) — в `MULTITENANCY.md`.

---

## 5. Движок методик (`public/methodologies.js`)

Единый подсчёт: `window.scoreMethodology(meth, answers)` → `{ scales:[{id,name,raw,maxScore,interp}], total, validity, primary }`.
- Поддерживает: **подшкалы** (`scales[]`), пересекающиеся шкалы (`scale: ['id1','id2']`),
  **обратные вопросы** (`reverse: true` — вес зеркальный `min+max−score`), среднее по шкале
  (`calculation:'average'`), итоговые/агрегатные шкалы, **шкалы достоверности** (`validity[]`)
  и свои варианты у вопроса.
- Одношкальный случай (вопросы-строки + `interpretation` + `maxScore`) — обратно совместим.
- `window.resultAttention(meth, answers)` → попал ли результат в диапазон с флагом `attention: true`
  (это и есть «группа риска» — задаётся самой методикой).
- `findMethodologyByTitle / findMethodologyById`, `allMethodologies()` (встроенные + пользовательские из БД).
- Формулы, веса и результаты студенту не выдаются: подсчёт выполняется только на сервере,
  а результаты доступны психологу и куратору в пределах их полномочий.

**Формат методики** (объект в `window.PSY_METHODOLOGIES`):
```js
{
  id, title, description, instruction,
  answerOptions: [{ text, score }],          // общие варианты (вес)
  questions: [                                // строка ИЛИ объект:
    'Текст вопроса',
    { text, scale?, reverse?, options? }      // scale — id или массив id; reverse — обратный
  ],
  // одношкальная:
  maxScore, interpretation: [{ min, max, level, label, color?, attention? }],
  // ИЛИ многошкальная:
  scales: [{ id, name, maxScore?, aggregate?, calculation?, multiplier?, display?, interpretation:[...] }],
  totalScale,                                // необязательно: какую шкалу сохранять как score
  validity: [{ id, name, scale, threshold, warning }]   // необязательно
}
```
- `attention:true` определяет группу риска и тревожный цвет. Нельзя считать любой `high` риском:
  высокий уровень благополучия или жизнестойкости является положительным результатом.
- **Принцип масштабирования (важно):** НЕ хардкодить логику/инфографику под одну методику.
  Всё строй из данных `methodologies.js` — любое число шкал/уровней с любыми названиями. (Историческая
  ошибка: инфографику результатов однажды зашили под одну методику — переделали на data-driven.)
- Пользовательские методики психолог создаёт в редакторе (раздел «Методики» в `admin.html`),
  хранятся в БД (таблица `methodologies`, JSON `data`), CRUD — `/api/methodologies`.

---

## 6. Как ДОБАВИТЬ методику из PDF-сборника

Источник: `СБОРНИК диагностических методик РУСС.pdf` (у пользователя; 83 стр., ~24 методики).

Для работы с PDF сначала загрузи зависимости рабочего окружения. Текст страниц извлекается через
встроенный Python + `pypdf`, а таблицы и ключи обязательно перепроверяются визуально: нужные страницы
рендерятся встроенным Poppler (`pdftoppm`) в PNG и просматриваются как изображения. Временные PNG
после проверки удалить. Не полагаться только на OCR/извлечённый текст: в таблицах встречаются переносы
и опечатки, способные изменить ключ.

**Процесс:** отрендерь страницы методики → прочитай вопросы, варианты-веса и ключ/нормы →
оформи объект по формату §5 → добавь в `PSY_METHODOLOGIES` → **юнит-тест подсчёта** (см. §8) →
проверь §8 → коммит. **Точность ключей критична** — это клиника; проси пользователя сверить с источником.

**Уже добавлены (11 встроенных):** Рассел, Корчагина, Хмельницкая, Мадди, Янг, Олвеус,
Рифф, Бек, Кук—Медли, Розенберг, Кожухарь (подростки).

---

## 7. i18n RU/KZ — правила

- Текст в HTML тегается `data-i18n="ключ"` (textContent), `data-i18n-ph` (placeholder), `data-i18n-html` (innerHTML).
- Динамика в JS — через глобальную `t('ключ')`. Язык переключает `toggleLang()`; страница может задать `window.onLangChange`.
- **Каждый используемый ключ ОБЯЗАН быть и в `ru`, и в `kz`** в `public/i18n.js`. Паритет проверяется (см. §8).
- `platform.html` — на русском без i18n (внутренний кабинет супер-админа), это нормально.

---

## 8. Проверка перед коммитом — ОБЯЗАТЕЛЬНО

```bash
# 1. Синтаксис
node --check server.js && node --check public/*.js

# 2. Юнит-тесты
npm test

# 3. В HTML нет inline-скриптов/обработчиков + паритет i18n + нет пропущенных/лишних ключей.
#    Суть проверки: извлечь ключи ru/kz из i18n.js, собрать использованные (data-i18n* и t('...'))
#    в HTML и соответствующих page-JS, убедиться что нет missing и нет RU-only/KZ-only.
#    (Готовый скрипт многократно использовался — см. историю; уровни level.low/medium/high
#     используются динамически как t('level.'+x), статически могут выглядеть «unused» — это норма.)

# 4. Полный интеграционный прогон на отдельной MySQL
npm run test:integration

# 5. Старт production-процесса и HTTP
pm2 status
curl http://127.0.0.1:3000/  # -> 200
```
Если добавлял методику — обязательно прогони юнит-тест `scoreMethodology` (граничные значения,
обратные вопросы, шкалы достоверности). Примеры тестов есть в истории коммитов.

`npm run test:integration` использует настройки подключения из `.env`, создаёт только БД с префиксом
`psyagent_it_`, проверяет чистую и старую схемы и удаляет временные БД в `finally`. Запускать его только
с тестовой/локальной MySQL и учётной записью с правами `CREATE DATABASE` / `DROP DATABASE`; production-БД
он затрагивать не должен. Проверяются: миграции, изоляция колледжей, все 11 встроенных методик, вход,
назначение, защита от дубля, черновик, завершение, повторное назначение, фильтры и идемпотентный рестарт.

Для визуального QA проверять минимум: вход desktop/mobile, кабинеты admin/student/curator/platform,
модальное прохождение теста, сводный отчёт и печатное представление. Обязательно контролировать ошибки
консоли и горизонтальное переполнение. Browser-плагин предпочтителен; при его отсутствии допустим
временный Playwright вне репозитория, без добавления браузерных пакетов в runtime-зависимости проекта.

Если локально нет MySQL, ожидается безопасный отказ запуска с ненулевым кодом и без открытого HTTP-порта.
`dotenv` должен быть в `node_modules` (если нет — `npm install`).

---

## 9. Доступы, секреты, git

- **Супер-админ:** код колледжа `platform`, логин/пароль из `.env`
  (`SUPERADMIN_USER`/`SUPERADMIN_PASSWORD`). Пароль по умолчанию отсутствует.
- **Психолог-админ колледжа:** стандартной учётной записи нет; создаётся через `npm run create-admin`.
- **БД:** логин и уникальный сильный пароль задаются через `.env`:
  `DB_HOST/DB_USER/DB_PASSWORD/DB_NAME`, `CONTROL_DB_NAME`, `PORT`, `JWT_SECRET`.
- Для чистой установки скопировать `.env.example`, настроить MySQL и создать первого психолога
  командой `npm run create-admin`.
- `.env` — в `.gitignore`, **в репозиторий не коммитить**. Секреты в код не вписывать.
- **Git:** ветка `main`. Перед коммитом — проверки §8. Коммиты осмысленные. Пушить в `origin main`.

---

## 10. Текущие задачи (TODO)

1. **Методики из PDF** — оцифрованы все 11 студенческих опросников сборника, для которых
   источник содержит непротиворечивый балльный ключ и которые подходят текущей персональной модели.
   *НЕ оцифровываются (проективные/экспертные):* Дерево с человечками, Человек под дождём,
   Несуществующее животное, экспертные схемы адаптации, незаконченные предложения, таблица риска суицида
   Волковой и карта риска суицида Шнейдер (её заполняет педагог-психолог по итогам беседы).
   *ОТДЕЛЬНЫЙ МОДУЛЬ ВЫПОЛНЕН 2026-06-28:* 1.2.3/1.2.4 «Как с тобой обращаются»
   запускаются психологом по случайной публичной ссылке. Ответы хранятся без `user_id`, ФИО,
   логина и IP; психологу доступна только агрегированная статистика.
   *НУЖЕН ИСПРАВЛЕННЫЙ КЛЮЧ:* у 1.2.8 (Ракишева) напечатанные диапазоны перекрываются и не
   соответствуют возможной сумме по таблице, поэтому автоматическая интерпретация сейчас небезопасна.
   *НУЖЕН МОДУЛЬ ДЛЯ СОТРУДНИКОВ:* 2.2.3 Кожухарь предназначен для педагогов, а текущая система
   назначает диагностики только студентам. Не начинать этот модуль без отдельного указания владельца.
2. **Интеграционная проверка — выполнена 2026-06-27.** Добавлен воспроизводимый
   `npm run test:integration`; чистая схема, миграция старой схемы и полный сценарий прошли.
3. **Визуальная проверка — выполнена 2026-06-28.** Проверены desktop/mobile, анонимная форма,
   скрытие результатов студента, удаление колледжа и скачивание PDF студента/группы/колледжа;
   ошибок консоли и горизонтального переполнения нет.

**Отложено (по решению владельца):** тарифы/подписка; лендинг + демо-данные + триал; модуль согласий ПДн.

---

## 11. Что уже сделано (контекст)

Импорт/экспорт Excel, двуязычие RU/KZ, инфографика результатов (data-driven), движок методик
(подшкалы/обратные/достоверность), редактор своих методик (БД), скачивание PDF-заключений,
анонимные опросы, группа риска + динамика, мультиарендность (БД на колледж), роли
(супер-админ/админ/куратор/студент), выборочное управление пользователями и паролями,
отключение и безвозвратное удаление колледжей, ИИ-анализ результатов через Open Web UI
(по результату/студенту/группе, обезличенные промпты, кэш `ai_analyses`, секция в PDF).
Подробности — в `git log`.

---

## 12. Передача контекста между агентами

- Перед началом работы прочитать `AGENTS.md` и `HANDOFF.md`.
- Перед завершением каждой рабочей сессии обновить `HANDOFF.md`, даже если код не изменялся.
- Писать нейтрально и технически, без названий агентов, сервисов и личного контекста.
- Обязательно фиксировать: цель сессии, текущее состояние, внесённые изменения, затронутые файлы,
  выполненные проверки, известные проблемы и следующий рекомендуемый шаг.
- Если изменений не было, явно написать об этом. Не заявлять об успешной проверке, если она не запускалась.
- Не записывать в `HANDOFF.md` пароли, токены, содержимое `.env` и другие секреты.
- Не удалять незавершённые задачи без объяснения. Устаревшие сведения обновлять, а не накапливать.
- `HANDOFF.md` передаёт оперативный контекст, но фактическое состояние файлов определяется текущим
  рабочим деревом и `git diff`.
