...
# 3. Старт сервера с тестовой MySQL и настроенным .env
...
Полный интеграционный прогон выполняется на тестовой БД. `dotenv` должен быть в `node_modules`
(если нет — `npm install`).
...
2. **Интеграционная проверка** — развернуть тестовую MySQL, применить миграции и проверить полный
   сценарий: вход, создание теста из каждой встроенной методики, назначение, черновик, завершение,
   повторное назначение, фильтры и отчёты.
3. **Визуальная проверка** — проверить кабинеты в браузере на десктопе и мобильной ширине.
