...
> **Текущая работа (ветка `feat/multi-tenancy`):** перевод на мультиарендность —
> отдельная БД на каждый колледж + control-БД `psych_control`. Подробный статус,
> шаги настройки БД на сервере и что доделать/протестировать — в **`MULTITENANCY.md`**.
> Если работаешь на сервере с доступом к MySQL — начни с него.
